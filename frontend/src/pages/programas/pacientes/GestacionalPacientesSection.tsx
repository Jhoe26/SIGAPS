...
export function GestacionalPacientesSection({ senalNuevoRegistro }: SectionProps) {
  ...
  const filas: FilaClinica[] = (data?.content ?? []).map((r) => {
    ...
    return {
      ...
      fecha: r.createdAt,
      estado: r.esHistorico ? "inactivo" : aplicadas >= CAMPOS_VACUNA.length ? "activo" : "pendiente",
      estadoEtiqueta: r.esHistorico ? "Histórico" : `${aplicadas}/${CAMPOS_VACUNA.length} vacunas`,

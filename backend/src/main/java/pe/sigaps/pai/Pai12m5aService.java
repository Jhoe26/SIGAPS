...
@Service
public class Pai12m5aService extends AbstractPaiService<Pai12m5a> {

    public Pai12m5aService(Pai12m5aRepository repositorio, PacienteService pacienteService,
    ...
    @Override
    protected Pai12m5a nuevaInstancia() {
        return new Pai12m5a();
    }

    @Override
    protected String nombreEntidad() {
        return "Registro PAI 12m-5a";
    }

    @Transactional
    @Auditable(tabla = "pai_12m_5a", accion = AccionAuditoria.INSERT)
    public PaiResponseDto crear(CreatePaiDto dto) {
        return crearInterno(dto);
    }

    @Transactional
    @Auditable(tabla = "pai_12m_5a", accion = AccionAuditoria.UPDATE)
    public PaiResponseDto actualizar(Long id, UpdatePaiDto dto) {
        return actualizarInterno(id, dto);
    }

    @Transactional
    @Auditable(tabla = "pai_12m_5a", accion = AccionAuditoria.DELETE)
